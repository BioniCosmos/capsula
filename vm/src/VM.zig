...
pub const Instruction = enum(u8) { add, push };
...
pub fn addVar(self: *Self, variable: Var) !u16 {
    if (self.vars.items.len > math.maxInt(u16)) {
        self.err = fmt.comptimePrint(
            "adding variable to memory: max variable number exceeded, which is `{}`",
            .{math.maxInt(u16)},
        );
        return error.MaxVariableNumberExceeded;
    }
    try self.vars.append(self.allocator, variable);
